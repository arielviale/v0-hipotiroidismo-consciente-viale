...
export async function GET() {
  try {
    ...
    const { data: stories, error } = await supabase.from("stories").select("*").order("created_at", { ascending: false }).limit(50)

    if (error) {
      console.error("Supabase error fetching stories:", error)
      // If table doesn't exist, return empty array instead of 500?
      // Or 500 to indicate issue. Sticking to error behavior.
